...
LLM_WEIGHTED_SCORE=$(echo "scale=4; ($CORRECTNESS * 0.15 + $CONVENTION * 0.10 + $TEST_QUALITY * 0.10 + $COMMIT_QUALITY * 0.05 + $REVIEWER_READINESS * 0.10) / 0.50" | bc -l | sed 's/^\./0./')
LLM_NORMALIZED=$(echo "scale=4; $LLM_WEIGHTED_SCORE / 5" | bc -l | sed 's/^\./0./')

# Composite: 50% gate (0-1) + 50% LLM (0-1) → raw is 0-1
COMPOSITE_RAW=$(echo "scale=4; ($GATE_SCORE * 0.50) + ($LLM_NORMALIZED * 0.50)" | bc -l | sed 's/^\./0./')

# Scale to 0-5
COMPOSITE_SCORE=$(echo "scale=2; $COMPOSITE_RAW * 5" | bc -l | sed 's/^\./0./')

# Ensure score is within bounds (0-5)
COMPOSITE_SCORE=$(echo "scale=2; if ($COMPOSITE_SCORE < 0) 0 else if ($COMPOSITE_SCORE > 5) 5 else $COMPOSITE_SCORE" | bc -l | sed 's/^\./0./')
...
cat > "$OUTPUT" << EOF
{
  ...
  "formula": {
    ...
    "gate_component": $(echo "scale=4; $GATE_SCORE * 0.50" | bc -l | sed 's/^\./0./'),
    "judge_component": $(echo "scale=4; $LLM_WEIGHTED_SCORE * 0.50" | bc -l | sed 's/^\./0./')
  }
}
EOF
