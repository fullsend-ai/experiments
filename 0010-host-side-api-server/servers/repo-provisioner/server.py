...
def scan_repo(repo_dir: str) -> list[dict]:
    ...
    for root, dirs, files in os.walk(repo_dir):
        ...
        if rel_root == os.path.join(".git", "hooks") or rel_root.startswith(
            os.path.join(".git", "hooks") + os.sep
        ):
            for fname in files:
...
def main():
    ...
    parser.add_argument(
        "--bind-address", default="127.0.0.1", help="Address to bind to"
    )
    parser.add_argument(
        "--token", required=True, help="Bearer token for authentication"
    )
    parser.add_argument(
        "--sandbox", default="", help="OpenShell sandbox name for uploading repos"
    )
    parser.add_argument(
        "--config", default="", help="Path to optional config JSON file"
    )
