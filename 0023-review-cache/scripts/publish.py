...
def compute_resolved_findings(
    pr_number: int, db_path: str, current_findings: List[dict]
) -> List[dict]:
    """
...
def main():
    ...
    parser.add_argument(
        "--findings", required=True, help="Path to classified findings JSON"
    )
    parser.add_argument(
        "--output", required=True, help="Path to output GitHub comment markdown"
    )
    parser.add_argument(
        "--pr", type=int, help="PR number (optional, for computing resolved findings)"
    )
    parser.add_argument(
        "--db",
        default=".fullsend/review-memory.db",
